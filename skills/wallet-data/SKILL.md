...
license: MIT
name: wallet-data
description: Raw account/wallet data pull: balances, positions, transfer & fill history.
---

# wallet-data

Read-only account facts for journaling and portfolio skills.
...
## Inputs
Address or 'my account'

## Method
1. Balances across wallets (spot/funding/futures).
2. Open positions if futures account.
3. Fill/trade history window; transfer history window.

## Output contract (use these exact section headers)
WALLET DATA RETURN
- balances | positions | fills | transfers (as requested)
- SOURCE + timestamps; MISSING markers for gaps
