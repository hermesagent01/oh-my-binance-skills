...
description: Compare current research question against previous stored research.
---
...
## Method
Search your local notes/reports directory (configure path once in your
setup notes) for prior work on the subject. Diff old claims vs current data.

## Output contract (use these exact section headers)
RESEARCH MEMORY - $SUBJECT
PRIOR: report(s) + date + core claim
NOW: current data on those claims
DELTA: CONFIRMED | BROKEN | UNTESTED per claim
