...
description: Find similar historical conditions and what followed - sample-size honesty required.
---
...
## Inputs
Current setup description or symbol

## Method
Compare current structure+funding+OI fingerprint to known episodes
(model knowledge + web verification). Always include dissimilarities.

## Output contract (use these exact section headers)
HISTORICAL ANALOGY
CURRENT FINGERPRINT: structure+funding+OI snapshot
ANALOGS: episode, date, similarity, outcome
BASE RATE: small-n honesty
DISSIMILARITY: how now differs (never skip)
