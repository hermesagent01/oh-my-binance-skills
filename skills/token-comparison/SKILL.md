...
Compare 2-5 tokens across identical dimensions and window.
...
## Method
Run token-profile + token-health logic per token, then align rows.
Same lookback window for all (default 30d).

## Output contract (use these exact section headers)
COMPARISON - A vs B [vs C]
TABLE: perf | volume | liquidity | age | health verdict | key flag
EDGE: unique strength of each
VERDICT: structurally strongest + one line why

## Data source (this library)

Web3/on-chain evidence comes from the vendored official launcher(s):
`vendors/binance-web3/query-token-info (x2)/scripts/cli.mjs`

```
node vendors/binance-web3/query-token-info (x2)/scripts/cli.mjs search '{"keyword":"A"} then {"keyword":"B"}'
```
