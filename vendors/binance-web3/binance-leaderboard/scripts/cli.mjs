#!/usr/bin/env node
// binance-leaderboard CLI — official Binance Web3 leaderboard (baw dependency)
// Usage:
//   node cli.mjs leaderboard '<command>' [options...] --json → passthrough to `baw leaderboard ...`
...
if (!arg0 || arg0 === '--help' || arg0 === '-h') {
  console.log("Usage: node cli.mjs leaderboard '<subcommand>' [options] --json   (passthrough to baw)");
  console.log('       node cli.mjs --check');
  console.log('\nSubcommands: gem-hunter / gainers / ... see references/scoring.md');
  process.exit(0);
}
